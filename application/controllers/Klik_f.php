...
class Klik_f extends CI_Controller {
	...
	function cari_karyawan(){
		$kondisi = array(
			'kry_mst_ic' => $this->input->post('krymstic'),
			);
	...
	function tambah_karyawan_ok(){
		$nilai_kry_master = array(
			...
			'kry_mst_nm' => strtoupper($this->input->post('t_kry_mst_nm')),
			...
			'kry_mst_kel' => strtoupper($this->input->post('t_kry_mst_kel')),
		...
		switch ($this->input->post('btnKirim')){
			case "TAMBAH":
				...
				$this->form_validation->set_rules('t_kry_mst_nm','NAMA','required');
				...
				$this->form_validation->set_rules('t_kry_mst_kel','KELOMPOK','required');
