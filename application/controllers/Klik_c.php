...
class Klik_c extends CI_Controller {
	...
	function __construct(){
		...
		$this->FormC1 = "daftar_c1";
		$this->KePilihanC1 = "klik_c/pilihan_c1";
	...
	function index(){
		if(($this->session->userdata('hak') == "PENGAWAS") || ($this->session->userdata('hak') == "PELAKSANA")){
			...
			redirect($this->KePilihanC1);
		}
		
	}

	function kosong_operator_validasi(){
		$this->session->unset_userdata('operator_c1');
		$this->session->unset_userdata('validasi_c1');
	}
	...
	function tambah_hutang_ok(){	
		if ($this->input->post('btnKirim')!="BATAL"){
			...
			switch ($this->input->post('t_hut_jenis')){
				...
				default:
					$validasi_c1 = array(
						'validasi_c1' => "jenis kegiatan tidak terdaftar atau belum di pilih"
					);
					$this->session->set_userdata($validasi_c1);
					redirect($this->KePilihanC1);
					$this->kosong_operator_validasi();
					break;
			}
			...
			if($this->form_validation->run() != false){
				if (!$this->upload->do_upload('t_hut_mst_doc')){
					$validasi_c1 = array(
						'validasi_c1' => "Unggah proposal ga berhasil, PROGRAM=berkas doc/docx, BIAYA RUTIN=berkas jpeg"
					);
					$this->session->set_userdata($validasi_c1);
					redirect($this->KePilihanC1);
					$this->kosong_operator_validasi();
				...
				$this->m_db->tambah_data($nilai_master,$this->TabelHutangMaster);
					...
					$validasi_c1 = array('validasi_c1' => validation_errors('<li>','</li>'));
					$this->session->set_userdata($validasi_c1);
			}
		}
		redirect($this->KePilihanC1);
	...
	function hapus_hutang_ok($hutprm){
		...
		if (empty($data['daftar_hutang_master'])){
			$validasi_c1 = array(
				'validasi_c1' => "Pengajuan anggaran sudah selesai di proses, nda boleh hapus data yang sudah masuk..."
			);
			$this->session->set_userdata($validasi_c1);
		} else {
